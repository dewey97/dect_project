    ...
    >
      ...
      {selectedThread ? (
        <div className="flex flex-col h-full animate-in slide-in-from-right-4 duration-200">
          ...
          {/* Messages Stream */}
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2 flex flex-col justify-start pb-10">
            {selectedThread.messages.map((msg: Message, idx: number) => {
              const isMe = msg.role === 'sent'
              const isFirstOfGroup =
                idx === 0 ||
                selectedThread.messages[idx - 1]?.role !== msg.role ||
                selectedThread.messages[idx - 1]?.timestamp !== msg.timestamp

              const isPlayingThis = playingAudioId === msg.id

              return (
                <div key={msg.id} className="space-y-1">
                  {/* Date & Time Header (Centered above message group) */}
                  {isFirstOfGroup && (
                  ...
                  >
                    {/* Chat Bubble Container - Pure Message Content Only */}
                    <div
                      className={cn(
                        'px-3.5 py-2.5 rounded-[18px] text-[13px] leading-relaxed break-words shadow-sm transition-all relative',
                        isMe
                    ...
                    >
                      ...
                      {msg.attachment?.type === 'audio' && (
                        <div className="mb-2 p-2 rounded-xl bg-black/30 border border-white/10 flex items-center gap-2.5">
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
                              togglePlayAudio(msg)
                          ...
                          <div className="flex-1 min-w-0 font-mono text-[10px] text-white/90">
                            <span>Ghi âm thoại ({msg.attachment.duration || '0:18'})</span>
                          </div>
                        </div>
                      )}

                      {/* Pure Message Text Content */}
