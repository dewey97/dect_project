...
export function IntegrityIndicator({
  ...
  const INTEGRITY_TRANSLATIONS = {
    secured: 'NGUYÊN VẸN // ĐÃ NIÊM PHONG',
    analyzing: 'ĐANG XỬ LÝ',
    corrupted: 'BỊ THẤT LẠC'
  }

  return (
    <div className={cn(
      'rounded-lg border border-border/50 bg-card/20 p-3.5 flex flex-col gap-2 font-sans text-[0.65rem] text-muted-foreground',
      className
    )}>
      <div className="flex items-center justify-between border-b border-border/20 pb-1.5">
        <span className="font-bold text-foreground tracking-wider uppercase flex items-center gap-1.5">
          <Shield className="size-3.5 text-primary" /> BIÊN BẢN GHI NHẬN & BẢO QUẢN VẬT CHỨNG
        </span>
        <span className={cn(
          ...
          {INTEGRITY_TRANSLATIONS[integrityStatus]}
        </span>
      </div>

      <div className="grid grid-cols-2 gap-2 text-xs">
        <div>
          <span className="block text-[0.6rem] text-muted-foreground/75 uppercase font-semibold">Điều tra viên tiếp nhận:</span>
          <span className="text-foreground font-semibold">{recoveredBy}</span>
        </div>
        <div className="text-right">
          <span className="block text-[0.6rem] text-muted-foreground/75 uppercase font-semibold">Thời gian thu giữ:</span>
      ...
      <div className="mt-1 flex items-center gap-1.5 text-[0.6rem] font-bold text-emerald-500 border-t border-border/10 pt-1.5 justify-between">
        <span>TÌNH TRẠNG VẬT LÝ: ĐÃ XÁC THỰC NIÊM PHONG</span>
