...
export function CeilingProps() {
  ...
  return (
    <group position={[0, 3.4, 0]}>
      ...
      <group position={[0, -0.25, 0.5]}>
        ...
        <group ref={fanGroupRef}>
          ...
          {[0, (Math.PI * 2) / 3, (Math.PI * 4) / 3].map((rad, i) => (
            <mesh key={i} rotation={[0.08, rad, 0]} position={[Math.cos(rad) * 0.55, 0, Math.sin(rad) * 0.55]} castShadow>
      ...
      {/* 3. CENTRAL ROOM OVERHEAD LIGHT (CHIẾU TỎA ĐỀU & ĐỔ BÓNG CHÍNH XÁC TOÀN BỘ PHÒNG TẠI X = -1.1, Z = 0) */}
